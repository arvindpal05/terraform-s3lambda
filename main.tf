variable "lambda_function_name" {
  default = "arvinds3lambdaevent"
}

locals {
  s3bucketarn = "arn:aws:s3:::tf-s3lambdaevent-arvind-bucket"
}

resource "aws_s3_bucket" "lambda_trigger_bucket" {
  bucket = "tf-s3lambdaevent-arvind-bucket"
}

resource "aws_iam_role" "iam_for_lambda" {
  name = "arvind_iam_for_lambda"
